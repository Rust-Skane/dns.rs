...
pub fn unpack(message: &[u8], offset: uint) -> Option<(Resource, uint)> {
  ...
  let rdata = match ty {
    ...
    _ => {
      let data = message[o .. o+ rlength as uint].to_vec();

      ResourceData::Raw(data)
    }
  };

  return Some((Resource { name: name, rrtype: ty, class: cl, ttl: ttl, rlength: rlength, rdata: rdata }, o + rlength as uint));
}
